...
ratings_database_path = xbmcvfs.translatePath(
    "special://profile/addon_data/script.fentastic.helperr/ratings_cache.db"
)
IMAGE_PATH = "special://home/addons/skin.fentasticp/resources/rating_images/"
...
def play_trailer():
    if not xbmc.getCondVisibility("System.HasAddon(plugin.video.imdb.trailers)"):
        dialog.notification(
            "FENtastic Plus",
            "IMDb Trailers add-on is not installed.",
            "",
            3000,
        )
        return

    imdb_id = xbmc.getInfoLabel("ListItem.IMDBNumber") or xbmc.getInfoLabel(
        "ListItem.UniqueID(imdb)"
    )
    if not imdb_id or not imdb_id.startswith("tt"):
        dialog.notification(
            "FENtastic Plus",
            "No IMDb ID available for this item.",
            "",
            3000,
        )
        return

    play_url = "plugin://plugin.video.imdb.trailers/?action=play_id&imdb=" + imdb_id
    season = xbmc.getInfoLabel("ListItem.Season")
    if season and season.isdigit() and int(season) > 0:
        play_url += "&season=" + season
