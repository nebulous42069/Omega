...
class SkinSettings:
    '''allows skinners to have custom dialogs for their skin settings and constants'''
    params = {}
    skinsettings = {}
                ...
                for resolution in resolutions:
                    if sys.version_info.major == 3:
                        ...
                        os.path.join(
                            ...
                            "script-bingie_toolbox-includes.xml").encode("utf-8"))
                    ...
                    if sys.version_info.major == 3:
                        includes_file = try_decode(xbmcvfs.translatePath(
                            try_encode(os.path.join(
                                ...
                                try_decode(
                                    resolution.attrib.get("folder")),
                                "script-bingie_toolbox-includes.xml"))))
    ...
    def get_skin_settings():
        ...
        all_skinsettings = {}
        settings_file = try_decode(xbmcvfs.translatePath('special://skin/extras/bingiesettings.xml'))
    ...
    def save_skin_image(self, skinstring="", multi_image=False, header=""):
        ...
        if not multi_image:
            ...
            if not cur_value_org.startswith("$"):
                ...
                curdir = ""
            value = try_decode(xbmcgui.Dialog().browse(0, self.addon.getLocalizedString(32003),
    ...
    def select_image(self, skinstring, allow_multi=True, windowheader="",
                     resource_addon="", current_value=""):
        '''lets the user select an image or imagepath from resourceaddons or custom path'''
        images = []
        if not windowheader:
            windowheader = self.addon.getLocalizedString(32011)
        if not current_value:
            current_value = try_decode(xbmc.getInfoLabel("Skin.String(%s.label)" % skinstring))

        # none option
        images.append((self.addon.getLocalizedString(32000), "", "", "DefaultAddonNone.png"))
        # custom single
        images.append((self.addon.getLocalizedString(32002), "", "", "DefaultAddonPicture.png"))
        # custom multi
        if allow_multi:
            images.append((self.addon.getLocalizedString(32003), "", "", "DefaultFolder.png"))
        ...
        if isinstance(result, bool):
            ...
            if label == self.addon.getLocalizedString(32002):
                # browse for single image
                custom_image = SkinSettings().save_skin_image(skinstring, False, self.addon.getLocalizedString(32002))
                if custom_image:
                    result.setPath(custom_image)
                else:
                    return self.selectimage()
            elif label == self.addon.getLocalizedString(32003):
                # browse for image path
                custom_image = SkinSettings().save_skin_image(skinstring, True, self.addon.getLocalizedString(32003))
                if custom_image:
                    result.setPath(custom_image)
                else:
                    return self.selectimage()
    ...
    def indent_xml(self, elem, level=0):
        '''properly indent xml strings to file'''
        text_i = "\n" + level * "\t"
        if len(elem):
