...
class Indexer(Debrid):
	...
	def set_auth(self):
		...
		data = {'client_id': client_id, 'code': result['device_code'], 'grant_type': 'device_code'}
		try:
			qr_url = '&data=%s' % urllib.parse.quote(result['verification_uri'])
		...
		headers = {'User-Agent': user_agent, 'Authorization': 'Bearer %s' % token}
		result = session.get(f"{base_url}account/info", headers=headers, timeout=timeout).json()
		customer = result['customer_id']
		set_setting('pm.account_id', str(customer))
