...
def MENU():
    addDir('Local Radio Stations','-',2,addonicon,'','Sender aus deiner Region')
    addDir('Search Radio Stations','-',3,addonicon,'','Sender suchen')
    addDir('My Radio Stations','-',5,addonicon,'','Meine Sender')

def LOCAL(page):
    try:
        addDir('[COLOR yellow][B]Return to Menu[/B][/COLOR]','','',addonicon,'','')
        ...
        if offset < count:
            ...
            addDir('[COLOR yellow][B]Next Page[/B][/COLOR] [COLOR green][B]>>>[/B][/COLOR]',nextpage,2,addonicon,'','')
    except:
        xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%('[B]Error[/B]', 'Stream not found.', 5000, addonicon))
...
def ADDSTATION(url, name, image):
    ...
    with open(f"{favourites_list}", "a") as f:
        ...
        f.write("\n")
        f.close()
    xbmc.executebuiltin('Notification(%s, %s, %d, %s)'%('[B]'+name+'[/B]', 'Added radio to "My Radio Stations".', 5000, addonicon))
...
def addLink(link,name,image,desc,urlType,fanart):
    ...
    contextMenuItems = []
    contextMenuItems.append(('Add to "My Radio Stations"', f'RunPlugin(plugin://plugin.audio.radio_de_light/{add})'))
...
def addMy(link,name,image,desc,urlType,fanart):
    ...
    liz.setArt({'icon': image, 'thumb': image, 'poster': image, 'fanart': addonfanart})
    contextMenuItems = []
    contextMenuItems.append(('Remove to "My Radio Stations"', f'RunPlugin(plugin://plugin.audio.radio_de_light/{rem})'))
    liz.addContextMenuItems(contextMenuItems, replaceItems=True)
    ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=url,listitem=liz)
