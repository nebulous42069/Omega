...
mpdir         =  translatePath(os.path.join('special://home/userdata/addon_data/plugin.audio.mp3streams/temp_dl',''))
...
for dirpath, dirnames, filenames in os.walk(mpdir):
    count = 0
    for f in filenames:
        count += 1
        fp = os.path.join(dirpath, f)
...
if int(total_sizetext) > filesize:
    choice2 = xbmcgui.Dialog().yesno("[COLOR=red]Autocleaner[/COLOR]", 'The Mp3 Streams folder is [COLOR red]' + str(total_sizetext) +' MB [/COLOR] - [COLOR red]' + str(count) + '[/COLOR] zip files' + '\n' + 'The folder can be cleaned up without issues to save space...' + '\n' + 'Do you want to clean it now?', yeslabel='Yes',nolabel='No')
    if choice2 == 1:
        maintenance.purgemp()        
