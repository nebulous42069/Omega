...
def donate_menu():
    ...
    for i in [('Monero (XMR)', 'XMR_QR.png'),
              ('Bitcoin (BTC)', 'BTC_QR.png'),
              ('Bitcoin Legacy (BTC)', 'BTC_L_QR.png'),
        ...
        list_item.setInfo('video', {'title': 'Donate %s' % i[0],
                                    'genre': 'Donate %s' % i[0],
                                    'mediatype': 'video'})
        list_item.setArt({'icon': picture_path, 'thumb': picture_path})
...
def list_root():
    ...
    list_item.setInfo('video', {'title': 'Donate ...',
                                'genre': 'Donate ...',
                                'mediatype': 'video'})
    picture_path = os.path.join(xbmcaddon.Addon().getAddonInfo('path'), 'resources', 'donate', 'XMR_QR.png')
    list_item.setArt({'icon': picture_path, 'thumb': picture_path})
...
def list_videos(url):
    ...
    for video in videos:
        ...
        context_menu_download = ('Download video',
                         'RunPlugin({})'.format(url))

        url = get_url(action='browser', video_page=video['page'], video_name=video['name'])
        context_menu_open = ('Open video in browser',
                         'RunPlugin({})'.format(url))
...
def play_video(path):
    ...
    # xbmc.log("Playing %s ..." % video[1], xbmc.LOGERROR)
    # Pass the item to the Kodi player.
    xbmcplugin.setResolvedUrl(_handle, True, listitem=play_item)
