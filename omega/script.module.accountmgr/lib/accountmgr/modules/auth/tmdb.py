...
def _tmdb_api_key():
	key = control.setting('tmdb.api.key')
	if key == '' or key is None:
		return '05a454b451f2f9003fbca293744e4a85'
	return key
...
class Auth:
	...
	def create_session_id(self):
		try:
			...
			url = self.auth_base_link + '/token/new?api_key=%s' % _tmdb_api_key()
			result = requests.get(url, timeout=15).json()
			token = result.get('request_token')
			url2 = self.auth_base_link + '/token/validate_with_login?api_key=%s' % _tmdb_api_key()
			...
			url3 = self.auth_base_link + '/session/new?api_key=%s' % _tmdb_api_key()
	...
	def revoke_session_id(self):
		try:
			...
			url = self.auth_base_link + '/session?api_key=%s' % _tmdb_api_key()
