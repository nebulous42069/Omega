...
def process(provider, context, **_kwargs):
    ...
    Video:
    plugin://plugin.video.youtubek/play/?video_id=<VIDEO_ID>

    * VIDEO_ID: YouTube Video ID

    Playlist:
    plugin://plugin.video.youtubek/play/?playlist_id=<PLAYLIST_ID>[&order=<ORDER>][&action=<ACTION>]
    ...
    * ACTION: [list|play|queue|None(default)] optional action to perform
    ...
    plugin://plugin.video.youtubek/play/?channel_id=<CHANNEL_ID>[&live=X]
