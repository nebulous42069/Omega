...
class source:
	...
	def results(self, info):
		try:
			...
			self.folder_query, self.year_query_list = source_utils.clean_title(normalize(title)), tuple(map(str, range(self.year - 1, self.year + 2)))
			self._scrape_cloud()
	...
	def _scrape_cloud(self):
		try:
			...
			try: my_cloud_files = self.Offcloud.user_cloud()
			...
			for item in my_cloud_files:
				...
				if item['isDirectory']: append(Thread(target=self._scrape_folders, args=(item['requestId'],)))
					...
					if not item['fileName'].endswith(tuple(self.extensions)): continue
					match = False
					...
					elif source_utils.seas_ep_filter(self.season, self.episode, normalized): match = True
					if match: results_append({'filename': normalized, 'url': self.Offcloud.build_url(item['server'], item['requestId'], item['fileName'])})
	...
	def _scrape_folders(self, folder_info):
		try:
			results_append = self.scrape_results.append
			torrent_info = self.Offcloud.torrent_info(folder_info)
