...
def get_currently_active_model():
	...
	try:
		...
		if timeout_models: model_id = next(i for i in model_order if i not in (x['model_id'] for x in timeout_models if x['timeout_ends'] >= current_time))
		else: model_id = model_order[0]
	except: pass
	return model_id

def set_currently_active_models(model_id, timeout):
	try:
		timeout_models = lists_cache.get('ai_model_failed') or []
		timeout_models.append({'model_id': model_id, 'timeout_ends': get_timestamp(timeout)})
