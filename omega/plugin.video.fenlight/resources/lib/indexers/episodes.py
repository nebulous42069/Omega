...
def build_episode_list(params):
	def _process():
		for item in episodes_data:
			try:
				...
				info_tag.setCast([kodi_actor(name=item['name'], role=item['role'], thumbnail=item['thumbnail']) for item in cast + item_get('guest_stars', [])])
	...
	cast, mpaa, trailer, genre, studio, country = meta_get('cast', []), meta_get('mpaa'), str(meta_get('trailer')), meta_get('genre'), meta_get('studio'), meta_get('country')
...
def build_single_episode(list_type, params={}):
	...
	def _process(_position, ep_data):
		try:
			...
			orig_title, rootname, trailer, genre, studio = meta_get('original_title'), meta_get('rootname'), str(meta_get('trailer')), meta_get('genre'), meta_get('studio')
	...
	if list_type == 'episode.next':
		...
		hidden_data = ws.get_hidden_progress_items(watched_indicators)
		...
		if include_unwatched != 0:
			if include_unwatched in (1, 3):
				try: unwatched.extend([{'media_ids': i['media_ids'], 'season': 1, 'episode': 0, 'unwatched': True, 'title': i['title']} for i in trakt_watchlist('tvshow', '')])
				except: pass
	...
	[i.join() for i in TaskPool().tasks_enumerate(_process, data, min(len(data), settings.max_threads()))]
